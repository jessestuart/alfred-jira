...
const appName = appDetails.name.replace(
  /([a-z])([a-z]+)/g,
...
app.on('ready', function() {
      ...
      dialog.showMessageBox(
        {
          ...
          detail:
            'In order for the changes to take effect, you may need to restart Alfred.',
          title: app.getName(),
          icon: icon,
  ...
  app.on('before-quit', () => {
    ...
    sh.execSync(
      `find ${tmp.dir} -maxdepth 1 -type f -name '${tmp.prefix}*png' -delete`
    );
  });
});
...
ipcMain.on('get-option', (event, option) => {
  if (!loginOnly) {
    if (option == 'enabled_menu_items') {
      ...
      method()
        .then(data => {
          if (data) event.sender.send('set-option', option, data);
        })
        .catch(console.error);
    }
  }
  ...
  dialog.showOpenDialog(
    ...
    res => {
      if (res) {
        ...
        let tmpFile =
          path.normalize(path.join(tmp.dir + tmp.prefix)) +
          new Date().getTime() +
          '.png';
        sh.exec(
          `qlmanage -t -s 48 -o "${tmp.dir}" "${res}" 1>&2
