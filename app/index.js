...
Object.prototype.Get = function(key) {
  return key.split('.').reduce(function(s, p) {
...
app.controller('ctrl', [
  ...
  '$location',
  '$anchorScroll',
  ($scope, $timeout, $element, $location, $anchorScroll) => {
    ...
    $scope.login = () => {
      ...
      $scope.data.url =
        protocol + $scope.loginData.url.replace(/(.)\/*$/, '$1/');
    ...
    $scope.save = () => {
      ...
      if ($scope.data.bookmarks) {
        $scope.data.bookmarks = $scope.data.bookmarks.map((bookmark, index) => {
          let dest = config.cfgPath + 'bookmark-' + index + '.png';
          if (
            bookmark.icon &&
            bookmark.icon != dest &&
            !/resources/.test(bookmark.icon)
          ) {
      ...
      fs.writeFile(cfgFile, JSON.stringify($scope.data, null, 2), err => {
        ...
        if (bgCache !== undefined && bgInterval !== null) {
          ...
          if (bgCache && $scope.data.url) {
            daemonStatus = daemon.load($scope.data.url, bgInterval * 60)
              ? 'Background process running every ' +
                getTime(bgInterval * 60000)
        ...
        let text = document.body.getElementsByClassName('save')[0].lastChild
          .textContent;
        document.body.getElementsByClassName('save')[0].lastChild.textContent =
          'SAVED!';
        setTimeout(() => {
          document.body.getElementsByClassName(
            'save'
          )[0].lastChild.textContent = text;
        }, 1000);
      });
    };
    ...
    $scope.sortFields = pos => {
      return [
        ...
        return (
          ele == pos ||
          !$scope.selectedBookmark.sort.map(s => s.name).includes(ele)
        );
      });
    };
    ...
    const getTime = mil => {
      ...
      return [d, h, m, s]
        .map((time, index) => {
          ...
          return time
            ? time + ' ' + ['days', 'hours', 'minutes', 'seconds'][index]
            : 0;
        })
        .filter(Boolean)
        .join(' ');
    ...
    $scope.selectAllLabel = category =>
      !$scope.options[category].every(opt => opt.enabled)
        ? 'Select All'
        : 'Deselect All';
    ...
    if (fs.existsSync($scope.logFile)) {
      const updateDaemonLog = () =>
        $timeout(() => {
          try {
            $scope.daemonLog = sh
              .execSync(`tail -r -n 20 ${$scope.logFile}`)
              .toString();
          } catch (e) {
    ...
    $scope.$watch(
      'selectedBookmark.cache',
      val => ($scope.cacheConversion = getTime(val))
    );

    $scope.$watch(
      'options.backgroundCacheInterval',
