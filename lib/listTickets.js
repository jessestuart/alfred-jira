...
module.exports = {
      ...
      tickets
        ...
        .map(ticket => {
          return {
            ...
            valid: false,
            autocomplete:
              wf._sep + 'tickets ' + wf._sep + ticket.Key + ' ' + wf._sep,
  ...
  users: query => {
    Jira.getUsers().then(users => {
      wf.addItems(
        users.filter(s => new RegExp(query, 'i').test(s.name)).map(user => {
          return {
            title: user.name,
            valid: false
          };
  ...
  getTicket: function(ticket) {
    ...
    menu.push({
      ...
      arg: startProgress
        ? `startProgress ${ticket.Key}`
        : `stopProgress ${ticket.Key}`,
      valid: true,
      icon: startProgress ? 'play.png' : 'stop.png',
      wfId: 'progress',
      cmdMod: startProgress
        ? null
        : {
    ...
    if (ticket.Status) {
      menu.push({
        ...
        autocomplete:
          wf._sep + 'status ' + wf._sep + ticket.Key + ' ' + wf._sep,
        wfId: 'status'
      });
    }
  ...
  menu: function(query) {
    ...
    if (/bookmark/.test(context)) {
      ...
      wf.addItems(
        wf.storage
          .get(context)
          .filter(s => new RegExp(search, 'i').test(s.wfId))
      );
      wf.feedback();
    }
  }
