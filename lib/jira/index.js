...
Jira = {
  ...
  getAllBookmarks: function() {
    ...
    return Promise.all(
      self
        .getBookmarks()
        .map((bookmark, index) => self.listAll(`bookmark-${index}`))
    );
  },
    ...
    return new Promise((resolve, reject) => {
      self
        ...
        .catch(res => {
          ...
          if (!err) {
            reject(
              'Unable to receive response from: ' +
                config.url +
                ' ' +
                '\n' +
                res
  ...
  toggleWatch: function(ticket, currentState) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self
        .auth()
        .then(() => resolve(watch[['start', 'stop'][+currentState]](ticket)))
        .catch(reject);
    });
  },
  ...
  refreshCache: function() {
    let self = this;
    self.clearCache.apply(self, arguments).then(self.fetchData);
  },

  editSettings: function() {
    this.auth().then(() => {
      sh
        .spawn('npm', ['run', 'electron'], { detached: true, stdio: 'ignore' })
        .unref();
    });
  },
