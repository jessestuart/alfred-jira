...
function Item(settings) {
  ...
  icon = settings.projectIcon
    ? Icon(
        config.projectIconPath + settings.projectIcon,
        config.iconPath + 'default.png'
      )
    : icon;
  icon = settings.userIcon
    ? Icon(
        config.userIconPath + settings.userIcon,
        config.iconPath + 'assigned.png'
      )
    : icon;
  icon = settings.priorityIcon
    ? Icon(
        config.priorityIconPath + settings.priorityIcon,
        config.iconPath + 'priority.png'
      )
    : icon;
  icon = settings.bookmarkIcon
    ? Icon(settings.bookmarkIcon, config.iconPath + 'bookmark.png')
    : icon;
  ...
  if (alfredVersion > 2) {
    for (let modKey of ['ctrl', 'alt', 'cmd', 'fn', 'shift']) {
      ...
      if (modConfig && typeof modConfig == 'object') {
        ...
        if (modConfig.subtitle) mod['@subtitle'] = modConfig.subtitle;
        if (modConfig.arg) mod['@arg'] = modConfig.arg;
        mods.push(mod);
      }
    }
  }
  ...
  this.item = {
    ...
    '@autocomplete':
      settings.autocomplete !== undefined ? settings.autocomplete : null,
...
let Workflow = function() {
  return {
    ...
    error: function(title, subtitle, suppressFeedback) {
      ...
      if (!suppressFeedback) this.feedback();
    },
