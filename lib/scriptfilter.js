...
process.on('uncaughtException', function(error) {
  ...
  if (error.code === 'MODULE_NOT_FOUND') {
    let feedback =
      '<?xml version="1.0"?><items><item valid="no"><title>%s</title></item></items>';
...
actions.onAction('mainMenu', query => {
  ...
  let search = {
    title: 'Search Jira',
    valid: false,
    autocomplete: wf._sep + 'search ' + wf._sep + ' ' + query,
    icon: 'search.png'
  };
  if (Jira.checkConfig()) {
      ...
      wf.addItems(
        inProgress.map(issue => {
          return {
            ...
            valid: false,
            autocomplete:
              wf._sep + 'in-progress ' + wf._sep + issue.id + ' ' + wf._sep,
    ...
    wf.addItems(
      bookmarks.concat([
        search,
        {
          title: 'Settings',
          valid: false,
          autocomplete: wf._sep + 'settings ' + wf._sep,
          icon: 'config.png'
        }
      ])
    );
